...
/*--------------------------------------------
ROWNUM
1. dracle의 SELECT문 결과에 대해서 논리적인 일련번호를 부여한다.
...
3. rowid을 통해서 데이터가 어떤 데이터파일, 어브 블록에 저장되어 있는지 알수 있다.
4 . rowid 구조 (총 18자리)
오브젝트 번호(1~6) : 오브젝트별로 유일한 값을 가지고 있으며, 해당 오브젝트가 속해 잇는 값이다.
파일 번호(7~9) : 테이블스페이스에 속해 있는 데이터 파일에 대한 상대 파일번호이다.
블록번호(10~15) : 데이터 파일 내부에서 어느 블록에 데이터가 있는지 알려준다.
데이터번호(16~18) : 데이터 블록에 데이터가 저장되어 잇는 순서를 의미한다.
----------------------------------------*/
